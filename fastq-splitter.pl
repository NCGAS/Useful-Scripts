...
my ($opt_n_parts,$opt_part_size,$opt_measure,$opt_eol,$opt_out,$opt_check,$opt_version,$opt_help);
...
Options:
    ...
    --part-size <N>      - Divide into parts of size <N> (see measure for units)
    --measure (all|seq|count) - Specify whether all data (in bytes) , sequence length, or
...
sub split_file
{
    ...
    my $dir = '';
    my $newbase = $base;
    if ($base =~ /(.+)\/(.+)/) { ($dir,$newbase) = ($1,$2); }
    print "newbase is $newbase\n";
    if ($newbase =~ /^(.+?)(\.(f(?:ast)?q))$/i) { ($newbase,$ext) = ($1,$2); }
    
    print "$newbase, $dir, $ext, $infile";
    ...
    if ($part_size)
    {
        ...
        my $out_file = sprintf("%s/%s.part-%0*d%s",$dir,$newbase,$num_len,$part,$ext);
	print "outfile is $out_file\n";
        ...
        while (!eof $IN)
        {
            ...
            if ($this_part_size and ($this_part_size + $entry_size > $part_size))
            {
                ...
                $out_file = sprintf("%s/%s.part-%0*d%s",$dir,$newbase,$num_len,$part,$ext);
